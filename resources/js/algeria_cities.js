...
const algeria_cities = [
  ...
  $('.algeria_states').on('change',function(){
    const stateId =  $(this).val()
    const cities = algeria_cities.filter(city=>city.wilaya_code === stateId)
    $('.algeria_cities').html(
      $('.algeria_cities').first().html() +
      cities.map(city=>`<option value="${city.commune_name}">${city.commune_name}</option>`)
    )
  })
